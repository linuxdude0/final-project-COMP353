<html><head><style>table, th, td {border:1px solid black;}</style><title>LocationOperatingStaff</title></head><body><?php
...
$select_statement = 'SELECT * FROM LocationOperatingStaff';
$insert_statement = $conn->prepare('INSERT INTO LocationOperatingStaff(locID,staffID,startDate,endDate) VALUES (?,?,?,?)');
...
if(isset($_POST['delete_form_submit'])) {
	...
	try{
		if(!$conn->query("DELETE FROM LocationOperatingStaff WHERE ".$_POST['delete_select']." = ".$type."")) {
			die('huh????');
		}
		header('Refresh:0');
...
if(isset($_POST['update_form_submit'])) {
	...
	try{
	$conn->query("UPDATE LocationOperatingStaff SET ".$_POST['update_select_new_val']." = ".$a." WHERE ".$_POST['update_select_condition']." = ".$b."");
	header('Refresh:0');
