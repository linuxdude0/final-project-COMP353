<html><head><style>table, th, td {border:1px solid black;}</style><title>RegistrationAtLocation</title></head><body><?php
...
$select_statement = 'SELECT * FROM RegistrationAtLocation';
$insert_statement = $conn->prepare('INSERT INTO RegistrationAtLocation(locID,famID,startDate,endDate) VALUES (?,?,?,?)');
$insert_statement->bind_param('iiss', $_POST['locID'],$_POST['famID'],$_POST['startDate'],$_POST['endDate']);
...
if(isset($_POST['delete_form_submit'])) {
	...
	try{
		if(!$conn->query("DELETE FROM RegistrationAtLocation WHERE ".$_POST['delete_select']." = ".$type."")) {
			die('huh????');
		}
		header('Refresh:0');
...
if(isset($_POST['update_form_submit'])) {
	...
	try{
	$conn->query("UPDATE RegistrationAtLocation SET ".$_POST['update_select_new_val']." = ".$a." WHERE ".$_POST['update_select_condition']." = ".$b."");
	header('Refresh:0');
...
<form action='' method='POST'>
	...
	startDate:<input name='startDate'  type='date'  required><br>
