<html><head><style>table, th, td {border:1px solid black;}</style><title>ClubMemberPartOfTeam</title></head><body><?php
...
$select_statement = 'SELECT * FROM ClubMemberPartOfTeam';
$insert_statement = $conn->prepare('INSERT INTO ClubMemberPartOfTeam(clubID,teamID,roleOnTeam) VALUES (?,?,?)');
$insert_statement->bind_param('iis', $_POST['clubID'],$_POST['teamID'],$_POST['roleOnTeam']);
...
if(isset($_POST['delete_form_submit'])) {
	...
	try{
		if(!$conn->query("DELETE FROM ClubMemberPartOfTeam WHERE ".$_POST['delete_select']." = ".$type."")) {
			die('huh????');
		}
		header('Refresh:0');
...
if(isset($_POST['update_form_submit'])) {
	...
	try{
	$conn->query("UPDATE ClubMemberPartOfTeam SET ".$_POST['update_select_new_val']." = ".$a." WHERE ".$_POST['update_select_condition']." = ".$b."");
	header('Refresh:0');
...
<form action='' method='POST'>
	...
	teamID:<input name='teamID'  type='number'  required><br>
	roleOnTeam:<input name='roleOnTeam'  type='text' maxlength='255'  ><br>
...
Update: <form action='' method='POST'>
	<select name='update_select_condition' onchange='select_input_update_cond(event)'>
		...
		<option value='roleOnTeam'> roleOnTeam </option>
	</select>
...
<script>
	function select_input_delete(e) {
		switch(e.target.value) {
			...
			case 'roleOnTeam':
				document.getElementById('input_delete').innerHTML = 
					"<input name='delete_roleOnTeam'  type='text' maxlength='255'  required><br>";
			break;
		}
	}
	function select_input_update_new(e) {
		switch(e.target.value) {
			...
			case 'roleOnTeam':
				document.getElementById('input_update_new_val').innerHTML = 
					"<input name='update_new_val_roleOnTeam'  type='text' maxlength='255'  required><br>";
			break;
		}
	}
	function select_input_update_cond(e) {
		switch(e.target.value) {
			...
			case 'roleOnTeam':
				document.getElementById('input_update_condition').innerHTML = 
					"<input name='update_condition_roleOnTeam'  type='text' maxlength='255'  required><br>";
			break;
		}
	}
</script>
